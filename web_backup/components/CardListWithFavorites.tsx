...
export default function CardListWithFavorites({ 
  cards, 
  quantities, 
  onCardClick, 
  onAddCard, 
    ...
    return (
      <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 ${className}`}>
        {cards.map((card, index) => (
          <div 
            key={card.id} 
            className={`relative rounded-lg overflow-hidden transition-all hover:scale-[1.03] group favorite-card ${isFavorite(card.id) ? 'favorite-card-glow' : ''}`}
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {cards.map((card, index) => (
        <div 
          key={card.id}
          className={`flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer ${isFavorite(card.id) ? 'border-l-2 border-red-500' : ''}`}
          onClick={() => onCardClick?.(card)}
        >
