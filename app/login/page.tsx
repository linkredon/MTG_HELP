        ...
        try {
          ...
          if (user) {
            ...
            try {
              ...
              // Redirecionar para a página especificada ou para a página inicial
              if (redirectTo) {
    ...
    try {
      if (isLogin) {
        ...
        if (!result.success) {
          ...
          // Definir um cookie temporário para evitar loops de redirecionamento
          document.cookie = "mtg_auth_in_progress=true; path=/; max-age=10;"; // 10 segundos
          
          // Pequeno atraso para permitir que o cookie seja definido
          setTimeout(() => {
            // Redirecionar para a página especificada ou para a página inicial
            if (redirectTo) {
              console.log('Redirecionando para:', redirectTo);
        ...
        const isAdmin = adminCode === process.env.NEXT_PUBLIC_ADMIN_CODE || adminCode === 'MTG_ADMIN_2024';          // Registro
      ...
      <div className="w-full max-w-md bg-gray-800 rounded-lg shadow-xl p-6 space-y-6">
        ...
        <button
          ...
          onClick={() => {
            ...
            initDemoData();
            // Definir no localStorage e cookie para garantir que o modo de demonstração seja detectado
            localStorage.setItem('NEXT_PUBLIC_DEMO_MODE', 'true');
            document.cookie = "NEXT_PUBLIC_DEMO_MODE=true; path=/; max-age=86400"; // 24 horas
